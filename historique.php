...
<!--         <div class="car-image">
        ...
        try { 
            ...
            if (isset($_GET["nom_client"])) { // Vérifie si le nom du client est spécifié
                $nom_client = $_GET["nom_client"]; // Récupère le nom du client depuis l'URL
                ...
                $stmt = $connexion->prepare($requete); // Prépare la requête
                $nom_client = "%" . $nom_client . "%"; // Ajoute des % pour la recherche partielle
                $stmt->bindParam(':nom_client', $nom_client, PDO::PARAM_STR); // Lie le paramètre nom_client à la requête
                $stmt->execute(); // Exécute la requête

                $locations = $stmt->fetchAll(PDO::FETCH_ASSOC); // Récupère les résultats de la requête
                ...
                if (count($locations) > 0) {
                    ...
                    foreach ($locations as $location) { // Parcours des locations
                        echo "\t<tr>\n"; // Affiche les informations de chaque location
                        echo "\t\t<td>" . $location["id_client"] . "</td>\n"; // Affiche l'ID du client
                        echo "\t\t<td>" . $location["nom"] . "</td>\n"; // Affiche le nom du client
                        echo "\t\t<td>" . $location["prenom"] . "</td>\n"; // Affiche le prénom du client
                        if ($location["date_debut"]) { // Vérifie si le client a loué une voiture
                            ...
                            echo "\t\t<td>" . $location["compteur_debut"] . "</td>\n"; // Affiche le compteur de début de la location
                            echo "\t\t<td>" . $location["compteur_fin"] . "</td>\n"; // Affiche le compteur de fin de la location
                            echo "\t\t<td>" . $location["immatriculation"] . "</td>\n"; // Affiche l'immatriculation de la voiture
                        } else { // Affiche un message si le client n'a pas loué de voiture
                    ...
                    echo "<div class='no-client'>Aucun client trouvé avec ce nom.</div>"; // Affiche un message si aucun client n'est trouvé
                }
            } else {
                echo "<div class='no-client'>Nom du client non spécifié.</div>"; // Affiche un message si le nom du client n'est pas spécifié
            }
        } catch (PDOException $e) {
            echo "<div class='no-client'>Erreur : " . $e->getMessage() . "</div>"; // Affiche un message d'erreur
        }
        ?>
    </div>
</body>
