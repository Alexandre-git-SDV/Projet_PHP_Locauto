    ...
    <div class="container">
        ...
        try { 
            ...
            if (isset($_GET["nom_client"])) { // Vérifier si le nom du client est spécifié
                ...
                $requete = 'SELECT * FROM client WHERE nom LIKE :nom_client'; // Requête pour récupérer les informations du client
                $stmt = $connexion->prepare($requete); // Préparation de la requête
                $stmt->bindParam(':nom_client', $nom_client, PDO::PARAM_STR); // Remplacement du paramètre par la valeur
                ...
                while ($client = $stmt->fetch(PDO::FETCH_ASSOC)) { // Parcourir les résultats de la requête
                    echo "\t<tr>\n"; // Ligne du tableau
                    ...
                    $requete2 = 'SELECT * FROM type_de_client WHERE id_type_de_client = :id_type_de_client'; // Requête pour récupérer le type de client
                    $stmt2 = $connexion->prepare($requete2); // Préparation de la requête
                    $stmt2->bindParam(':id_type_de_client', $id_type_de_client, PDO::PARAM_INT); // Remplacement du paramètre par la valeur
                    $stmt2->execute(); // Exécution de la requête
                    $type_de_client = $stmt2->fetch(PDO::FETCH_ASSOC); // Récupérer le type de client
                    ...
                    $requete3 = 'SELECT organisation.nom FROM organisation
                                ...
                                WHERE appartient_a.id_client = :id_client'; // Requête pour récupérer l'organisation du client
                    $stmt3 = $connexion->prepare($requete3); // Préparation de la requête
                    $stmt3->bindParam(':id_client', $client["id_client"], PDO::PARAM_INT); // Remplacement du paramètre par la valeur
                    $stmt3->execute(); // Exécution de la requête
                    $organisation = $stmt3->fetch(PDO::FETCH_ASSOC); // Récupérer l'organisation du client

                    echo "\t\t<td>" . $organisation["nom"] . "</td>\n"; // Affichage de l'organisation du client
                    ...
                    $requete4 = 'SELECT location.date_debut, location.date_fin, location.compteur_debut, location.compteur_fin, voiture.immatriculation 
                                ...
                                WHERE location.id_client = :id_client'; // Requête pour récupérer les informations de la location du client
                    $stmt4 = $connexion->prepare($requete4); // Préparation de la requête
                    $stmt4->bindParam(':id_client', $client["id_client"], PDO::PARAM_INT); // Remplacement du paramètre par la valeur
                    $stmt4->execute(); // Exécution de la requête
                    $location = $stmt4->fetch(PDO::FETCH_ASSOC); // Récupérer les informations de la location du client
                    ...
                    if ($location) { // Vérifier si le client a loué une voiture
                        ...
                        echo "\t\t<td>" . $location["compteur_debut"] . "</td>\n"; // Affichage du compteur de début de la location
                        echo "\t\t<td>" . $location["compteur_fin"] . "</td>\n"; // Affichage du compteur de fin de la location
            ...
            } else {  // Si le nom du client n'est pas spécifié
                echo "<div class='no-client'>Nom du client non spécifié.</div>"; // Affichage d'un message d'erreur
            }
        } catch (PDOException $e) { // Gestion des erreurs
            echo "<div class='no-client'>Erreur : " . $e->getMessage() . "</div>"; // Affichage d'un message d'erreur
        }
        ?>
    </div>
</body>
